...
/*   Updated: 2021/03/16 12:54:16 by helvi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct termios *g_original = NULL;
...
void		enable_rawmode(t_editor *info)
{
	...
	g_original = info->original_termios;
	ft_atexit(&h_disable_rawmode);
}
...
static int	read_reply(char **buffer)
{
	...
	*buffer = ft_strnew(30);
	while (c != 'R')
	{
		...
		if (!ret || i >= 30)
		{
			ft_printf("cursor position reading failed.\n");
			ft_free(buffer);
			return (-1);
		}
		buffer[0][i++] = c;
